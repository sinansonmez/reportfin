...
__decorate([
    (0, type_graphql_1.Field)((_returns) => [FieldError], { nullable: true }),
    __metadata("design:type", Array)
], UserResponse.prototype, "errors", void 0);
__decorate([
    (0, type_graphql_1.Field)((_returns) => User_1.User, { nullable: true }),
...
let UserResolver = class UserResolver {
    ...
    async forgotPassword(email, { redis }) {
        ...
        await (0, sendEmail_1.sendEmail)(email, message);
        return true;
    }
    me({ req }) {
    ...
    async register(options, { req }) {
        ...
        const user = User_1.User.create({ username: options.username, password: hashedPassword, email: options.email });
        try {
            await user.save();
        }
        catch (e) {
    ...
    async login(usernameOrEmail, password, { req }) {
        ...
        const user = await User_1.User.findOne({ where: queryParam });
        if (!user) {
            return {
                errors: [{
